                    ...
                    if (e) {
                        ...
                        if (str = "", t) {
                            if ($) {
                                ...
                                !0 === $ && ($ = 130), i($) ? (r = n = $ + "px", $ = "s" + $ + "-c") : (a = /^s(\d+)(\-[cp])?$/.exec($)) ? (r = a[1] + "px", n = a[2] ? a[1] + "px" : "auto") : (a = /^w(\d+)\-h(\d+)(\-[cp])?$/.exec($)) && (r = a[1] + "px", n = a[2] + "px"), str += '<p class="' + P + "-image " + (o ? "loading" : "no-image") + '">', str += o ? '<img alt="" src="' + e.media$thumbnail.url.replace(/\/s[0-9]+(\-c)?/,"/s200") + '" style="display:block;width:' + r +';">' : '<span class="img" style="display:block;width:' + r +';">', str += "</p>"
                            }
